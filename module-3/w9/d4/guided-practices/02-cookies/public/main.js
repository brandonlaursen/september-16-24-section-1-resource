...
//   const found = cookies.find((cookie) => cookie.startsWith("theme"));

const theme = getCookie("theme");

  if (theme) {
    // const theme = found.split("=")[1];
    setTheme(theme);
  }
}
...
function getCookie(key) {
  // console.log(document.cookie);// display-name=brandon; theme=brandon
  const cookies = document.cookie.split("; ");
...
function restoreName() {
  // const name = document.cookie.split('=')[1];// theme='themeName' -> ['theme', 'themeName'][1]

  const name = getCookie("display-name");

  if (name) {
    // const name = found.split("=")[1];
